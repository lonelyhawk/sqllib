...
AS
	BEGIN
		...
		SELECT
					@table_object_id = tb.object_id, @is_cdc_enabled = tb.is_tracked_by_cdc
		FROM
					sys.tables	tb
		INNER JOIN	sys.schemas s
			ON s.schema_id = tb.schema_id
		WHERE
					s.name = @source_schema
		...
		IF @is_cdc_enabled = 1
			BEGIN
				...
				EXEC sys.sp_cdc_help_change_data_capture
					@source_schema = @source_schema, @source_name = @source_name;
				...
				IF @suppressMessages = 0
					PRINT 'Current setup is: capture_instance:' + ISNULL(@old_capture_instance, '<undefined>') + ', role_name:'
						  + ISNULL(@cdc_role_name, '<undefined>') + ', filegroup_name:' + ISNULL(@cdc_filegroup_name, '<undefined>') + ', index_name:'
						  + ISNULL(@cdc_index_name, '<undefined>');

				-- check if there were two capture instances.
				SELECT	TOP ( 1 )	@cdc_capture_instance = capture_instance
				FROM	@cdc_change_table
				ORDER BY
						object_id ASC;

				-- if the names match it means there is no other entry. 
				IF @old_capture_instance IS NOT NULL
					BEGIN
						IF @cdc_capture_instance = @old_capture_instance
							SET @cdc_capture_instance = NULL;
						ELSE
		...
		DECLARE
			@Index INT, @Slice NVARCHAR(MAX), @String NVARCHAR(MAX);

		DECLARE @proposedColumns TABLE ( cname sysname NOT NULL );
			...
			BEGIN
				...
				WHILE @Index <> 0
					BEGIN
						...
						INSERT INTO @proposedColumns ( cname )
									SELECT	name
									FROM	sys.columns
									WHERE
						...
						IF @@ROWCOUNT = 0
							BEGIN
								SET @String = N'The field ' + @Slice + N' has not been found in the source table ' + @source_name
		...
		DECLARE
			@columnsDropped INT, @columnsRemained INT, @columnsAdded INT;
		SELECT
					@columnsDropped	 = SUM(IIF(ncl.cname IS NULL AND ocl.column_name IS NOT NULL, 1, 0)),
			...
			BEGIN
				IF @cdc_index_name IS NULL
					BEGIN
						...
						SELECT		TOP ( 1 )	@cdc_index_name = i.name
						FROM
									sys.indexes i
						INNER JOIN	(
							SELECT
									object_id, index_id, COUNT(1) AS columns_in_index
						...
						WHERE
									i.object_id = @table_object_id
									AND i.is_unique = 1
									AND i.is_disabled = 0
						ORDER BY
									i.is_primary_key DESC, i.auto_created ASC, i.type ASC, ic.columns_in_index;
		...
		BEGIN TRY
			...
			IF @columnsRemained + @columnsAdded > 0
				BEGIN
					EXEC @res = sys.sp_cdc_enable_table
			...
			IF @columnsRemained > 0
				BEGIN
					...
					DECLARE
						@listofcommonfields NVARCHAR(MAX) = N'__$start_lsn,__$end_lsn,__$seqval,__$operation', @formula NVARCHAR(MAX) = N'';

					SELECT	@formula = STRING_AGG('CAST((' + parts.part + ') as varbinary(1))', '+') WITHIN GROUP(ORDER BY
																													  parts.grp DESC)
					FROM	(
						SELECT
								grp, STRING_AGG(chunks.part, N'|') AS part
						FROM	(
							SELECT		DISTINCT
										ncl.column_ordinal / 8 AS grp,
										IIF(ocl.column_ordinal IS NOT NULL,
											N'IIF(__$operation IN (3,4) and sys.fn_cdc_is_bit_set(' + CONVERT(NVARCHAR(10), ocl.column_ordinal)
											+ N',__$update_mask)=0,0,' + CONVERT(NVARCHAR(10), POWER(2, -1 + ( ncl.column_ordinal % 8 ))) + N')',
											'0')			   AS part
							FROM
										@newColumnList ncl
					...
					SET @sql = N'INSERT INTO cdc.' + QUOTENAME(@cdc_capture_instance + N'_CT') + N'(' + @listofcommonfields + N',__$update_mask) SELECT '
							   + @listofcommonfields + N', IIF(__$update_mask is not null,' + @formula + N',null) as __$update_mask FROM cdc.'
							   + QUOTENAME(@old_capture_instance + N'_CT');
			...
			IF @old_capture_instance IS NOT NULL
				BEGIN
					...
					EXEC @res = sys.sp_cdc_disable_table
						@source_schema = @source_schema, @source_name = @source_name, @capture_instance = @old_capture_instance;
					IF @res <> 0
